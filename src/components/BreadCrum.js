...
const BreadCrum = () => {
  ...
  let {state= stateName, district=districtName, taluk=talukName, mainCategory='', newsCategory=''} = useParams();
    ...
    const clickHandler = (path) => {
        localStorage.setItem('district', path === district ? district : '');
        dispatch(updateDistrict(path === district ? district : ''));
        localStorage.setItem('taluk', path === taluk ? taluk : '');
        dispatch(updateTaluk(path === taluk ? taluk : ''));
    }
    return (
        <>
            {
                        ...
                        <ol role="list" className="flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8 py-4">
                            ...
                            </li>
                            {
                                pathList()?.map((path, index) => (
                                    <li>
                                        <div className="flex items-center">
                                            <Link onClick={() => clickHandler(path.replaceAll('%20', ' '))} to={`/${pathList().slice(0, index + 1).join('/')}`} className="text-sm hover:text-slate-600 sm:block" style={path.length > 50 ? textEllipse : {whiteSpace: "nowrap"}}>{path.replaceAll('%20', ' ')}</Link>
                                            {
